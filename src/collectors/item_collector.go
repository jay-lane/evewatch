...
var (
	ewItemCollectorExporter ItemCollectorExporter
)

type ItemCollectorExporter struct {
	EveConfig      EveWatchConfig
	EWMetricPrefix string

	LastTimestamp map[string]map[int]time.Time // index by domain, datacenterid
	DCRegistry    *prometheus.Registry
}

func NewItemCollector(r *prometheus.Registry, eveWatchMetricsConfig EveWatchConfig, ewMetricPrefix string, tstart time.Time, lookbackDuration time.Duration) *ItemCollectorExporter {

	ewItemCollectorExporter = ItemCollectorExporter{GTMConfig: gtmMetricsConfig, DCLookbackDuration: lookbackDuration}
	...
	itemMap := make(map[string]map[int]time.Time)
	for _, domain := range eveWatchMetricsConfig.Domains {
		...
		itemMap[domain.Name] = tStampMap
	}
	ewItemCollectorExporter.LastTimestamp = itemMap

	return &ewItemCollectorExporter
}
